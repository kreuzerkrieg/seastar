...
future<> client::make_request(const request& req, reply_handler& handle, std::optional<reply::status_type> expected, abort_source* as) {
    ...
    return with_connection([this, &req, &handle, as, expected] (connection& con) {
        ...
        return repeat_until_value([this, err = std::move(ex), retry_count, &req, &handle, as, expected]() mutable -> future<std::optional<bool>> {
                auto on_return = deferred_action([&retry_count]() noexcept { ++retry_count; });
                return _retry_strategy->should_retry(err, retry_count).then([this, err, &req, &handle, as, expected](bool retry) mutable -> future<std::optional<bool>> {
                    if (retry) {
