    ...
    return app.run(ac, av, [&] {
        ...
        return seastar::async([=] {
            ...
            if (https) {
                ...
                fmt::print("{} {}:443{}\n", method, e.addr_list.front().addr, path);
                cln = std::make_unique<http::experimental::client>(socket_address(e.addr_list.front().addr, 443), std::move(certs), host);
            } else {
                fmt::print("{} {}:80{}\n", method, e.addr_list.front().addr, path);
                cln = std::make_unique<http::experimental::client>(socket_address(e.addr_list.front().addr, 80));
            }
