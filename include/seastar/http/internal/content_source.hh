...
namespace seastar::httpd::internal {
...
public:
    content_length_source_impl(input_stream<char>& inp, size_t length) : _inp(inp), _remaining_bytes(length) {}
    ...
    future<temporary_buffer<char>> skip(uint64_t n) override {
        ...
        return _inp.skip(skip_bytes).then([] { return temporary_buffer<char>(); });
    }
...
class chunked_source_impl : public data_source_impl {
    class chunk_parser {
        enum class parsing_state : uint8_t { size_and_ext, body, trailer_part };
        ...
        size_t _current_chunk_length=0;
    ...
    public:
        ...
        future<consumption_result_type> operator()(temporary_buffer<char> data) {
            if (!_buf.empty() || _end_of_request || data.empty()) {
...
public:
    ...
    future<temporary_buffer<char>> get() override {
        return _inp.consume(_chunk).then([this]() mutable { return _chunk.buf(); });
    }
